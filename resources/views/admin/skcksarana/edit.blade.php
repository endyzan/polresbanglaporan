...
<div class="container">
    <div class="card">
        ...
        <div class="card-body">
            <form action="{{ route('admin.spktkegiatan.update', $spktkegaitan->id) }}" method="POST" enctype="multipart/form-data" class="edit-form">
                @csrf
                @method('PUT')
                <div class="form-group">
                    <label for="nama_polsek" class="form-label">Nama Polsek</label>
                    <input type="text" name="nama_polsek" class="form-control" value="{{ $spktkegaitan->foto_spktkegiatan }}" required>
                </div>
                <div class="form-group">
                ...
                <div class="form-group">
                    <label for="foto_spktkegiatan" class="form-label">Foto Bangunan</label>
                    <input type="file" name="foto_spktkegiatan" class="form-control">
                    @if($spktkegiatan->foto_spktkegiatan)
                        <img src="{{ asset('storage/' . $spktkegaitan->foto_spktkegiatan) }}" width="100" class="mt-2 img-preview">
                    @endif
                </div>
